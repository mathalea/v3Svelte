...
export const uuid = 'f684d'
export const ref = '6N10-6'
export default function sommmeFractionsDecimales() {
  Exercice.call(this) // Héritage de la classe Exercice()
  this.nbQuestions = 6
  this.besoinFormulaireTexte = [
    "Type des calculs (les fractions sont décimales et de même dénominateur)",
  ...
  this.nouvelleVersion = () => {
    ...
    for (let i = 0, texte, texteCorr, cpt = 0, a, b, c, reponseAMC, denAMC, numAMC, choix; i < this.nbQuestions && cpt < 50; ) {
      a = randint(2, 19)
      b = randint(2, 19, a)
      c = randint(2, 19, [a, b])
      choix = randint(1, 3)
      ...
      switch (listeTypeDeQuestions[i]) {
        case 1: // Somme de deux fractions décimales de même dénominateur
          ...
          switch (this.sup2) {
            case 2:
              texteCorr = `$${texFraction(b, denAMC)}+${texFraction(c, denAMC)}=${texFraction(numAMC, denAMC)}$`
              break
            default:
              texteCorr = `$${texFraction(b, denAMC)}+${texFraction(c, denAMC)}=${texFraction(numAMC, denAMC)}=${texNombrec(reponseAMC)}$`
              break
          }
          break
        ...
        case 3: // Somme d'un entier avec une somme de deux fractions décimales de même dénominateur, sans retenue
          ...
          switch (this.sup2) {
            case 2:
              texteCorr = `$${a}+${texFraction(b, denAMC)}+${texFraction(c, denAMC)}=${a}+${texFraction(b + c, denAMC)}=${texFraction(a * denAMC, denAMC)}+${texFraction(b + c, denAMC)}=${texFraction(
                numAMC,
                denAMC
              )}$`
              break
      ...
      switch (this.sup2) {
        case 1:
          ...
          break
        case 2:
          setReponse(this, i, new FractionX(numAMC, denAMC), { digitsNum: nombreDeChiffresDe(numAMC), digitsDen: nombreDeChiffresDe(denAMC) + 1, signe: false, formatInteractif: "fraction" })
          break
        case 3:
