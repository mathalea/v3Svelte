...
export default function DivisionEuclidienneMultiplesDiviseursCriteres() {
    ...
    for (
      ...
      switch (typeDeQuestion) {
        case 1: // plus grand reste dans une division euclidienne
          ...
          texteCorr = `Si on divise par ${diviseur}, il ne peut pas rester plus de ${
            diviseur - 1
          }, sinon c'est qu'on peut encore ajouter au moins 1 fois ${diviseur} dans le dividende et donc 1 au quotient.`
          break
        ...
        case 3: // caractérisation des multiples et diviseurs par le reste de la division euclidienne
          ...
          if (listeDiviseurs(dividende).length % 2 === 0) {
          ...
          texte += `${nombreAvecEspace(dividende)} = ${nombreAvecEspace(candidatsDiviseurs[0])}$\\times$${nombreAvecEspace(Math.trunc(dividende / candidatsDiviseurs[0]))} $+$ ${nombreAvecEspace(
            dividende % candidatsDiviseurs[0]
          )}`
          texte += "<br>"
          texte += `${nombreAvecEspace(dividende)} = ${nombreAvecEspace(candidatsDiviseurs[1])}$\\times$${nombreAvecEspace(Math.trunc(dividende / candidatsDiviseurs[1]))} $+$ ${nombreAvecEspace(
            dividende % candidatsDiviseurs[1]
          )}`
          texte += "<br>"
          texte += `${nombreAvecEspace(dividende)} = ${nombreAvecEspace(candidatsDiviseurs[2])}$\\times$${nombreAvecEspace(Math.trunc(dividende / candidatsDiviseurs[2]))} $+$ ${nombreAvecEspace(
            dividende % candidatsDiviseurs[2]
          )}`
          texte += "<br>"
          texte += `Sans calculer, dire si les nombres ${nombreAvecEspace(candidatsDiviseurs[0])}; ${nombreAvecEspace(candidatsDiviseurs[1])}; ${nombreAvecEspace(
            candidatsDiviseurs[2]
          )} sont des diviseurs de ${nombreAvecEspace(dividende)}. Justifier.`
          texteCorr = ""
          if (egal(dividende % candidatsDiviseurs[0], 0)) {
            // egal() est une fonction de JC pour éviter les problèmes de virgule flottante
            texteCorr += `Le reste de la division euclienne de ${nombreAvecEspace(dividende)} par ${nombreAvecEspace(candidatsDiviseurs[0])} vaut 0 donc ${nombreAvecEspace(
              candidatsDiviseurs[0]
            )} est un diviseur de ${nombreAvecEspace(dividende)}`
          } else {
            texteCorr += `Le reste de la division euclienne de ${nombreAvecEspace(dividende)} par ${nombreAvecEspace(candidatsDiviseurs[0])} ne vaut pas 0 donc ${nombreAvecEspace(
              candidatsDiviseurs[0]
            )} n'est pas un diviseur de ${nombreAvecEspace(dividende)}`
          }
          texteCorr += "<br>"
          if (egal(dividende % candidatsDiviseurs[1], 0)) {
            // egal() est une fonction de JC pour éviter les problèmes de virgule flottante
            texteCorr += `Le reste de la division euclienne de ${nombreAvecEspace(dividende)} par ${nombreAvecEspace(candidatsDiviseurs[1])} vaut 0 donc ${nombreAvecEspace(
              candidatsDiviseurs[1]
            )} divise ${nombreAvecEspace(dividende)}`
          } else {
            texteCorr += `Le reste de la division euclienne de ${nombreAvecEspace(dividende)} par ${nombreAvecEspace(candidatsDiviseurs[1])} ne vaut pas 0 donc ${nombreAvecEspace(
              candidatsDiviseurs[1]
            )} ne divise pas ${nombreAvecEspace(dividende)}`
          }
          texteCorr += "<br>"
          if (egal(dividende % candidatsDiviseurs[2], 0)) {
            // egal() est une fonction de JC pour éviter les problèmes de virgule flottante
            texteCorr += `Le reste de la division euclienne de ${nombreAvecEspace(dividende)} par ${nombreAvecEspace(candidatsDiviseurs[2])} vaut 0 donc ${nombreAvecEspace(
              dividende
            )} est divisible par ${nombreAvecEspace(candidatsDiviseurs[2])}`
          } else {
            texteCorr += `Le reste de la division euclienne de ${nombreAvecEspace(dividende)} par ${nombreAvecEspace(candidatsDiviseurs[2])} ne vaut pas 0 donc ${nombreAvecEspace(
              dividende
            )} n'est pas divisible par ${nombreAvecEspace(candidatsDiviseurs[2])}`
          }
          texteCorr += "<br>"
          break
        case 4: // vocabulaire diviseurs et multiples
          ...
          multiplicateurs = [
            ...
            randint(2, 9, [multiplicateurs[0], multiplicateurs[1], multiplicateurs[2]]),
          ]
          ...
          textesCorr[4] = `${n1} n'est ni un multiple ni un diviseur de ${n2} car ${n1}=${n2}$\\times$${Math.trunc(n1 / n2)}+${texteEnCouleur(n1 % n2)} et ${n2}=${n1}$\\times$${Math.trunc(
            n2 / n1
          )}+${texteEnCouleur(n2 % n1)}`
          textes[5] = `${p2} $\\ldots\\ldots\\ldots\\ldots$ ${p1}`
          textesCorr[5] = `${p2} n'est ni un multiple ni un diviseur de ${p1} car ${p1}=${p2}$\\times$${Math.trunc(p1 / p2)}+${texteEnCouleur(p1 % p2)} et ${p2}=${p1}$\\times$${Math.trunc(
            p2 / p1
          )}+${texteEnCouleur(p2 % p1)}`
        ...
        case 5:
          if (nbDiviseursMax[i] > 10) {
            // les nombres de 2 chiffres ayant plus de 10 diviseurs étant peu nombreux, on force au moins 3 chiffres.
          ...
          texteCorr += `Il est suffisant de chercher des diviseurs inférieurs au plus grand nombre dont le carré vaut ${M}, par exemple ici, ${Math.trunc(Math.sqrt(M))}$\\times$${Math.trunc(
            Math.sqrt(M)
          )} = ${Math.trunc(Math.sqrt(M)) * Math.trunc(Math.sqrt(M))}<${M}`
          texteCorr += ` et ${Math.trunc(Math.sqrt(M)) + 1}$\\times$${Math.trunc(Math.sqrt(M)) + 1} = ${
            (Math.trunc(Math.sqrt(M)) + 1) * (Math.trunc(Math.sqrt(M)) + 1)
          }>${M} donc il suffit d'arrêter la recherche de facteur à ${Math.trunc(Math.sqrt(M))}.`
  ...
  this.besoinFormulaireNumerique = ["Nombres de chiffres de l'entier de la question 5", 10]
  this.besoinFormulaire2Numerique = ["Nombre maximum de diviseurs de l'entier de la question 5", 10]
}
