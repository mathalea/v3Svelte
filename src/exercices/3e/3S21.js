...
function unePieceDeuxUrnes(exercice, NoQuestion, sup, sup2, sup3) {
  ...
  texte +=
    `${numAlpha(q)} On vient de faire «${pileOuFace}». Donner la probabilité d'obtenir une boule ${boules[choix1]}.` + ajouteChampTexteMathLive(exercice, NoQuestion, "largeur15 inline") + "<br>"
  q++
  texte +=
    `${numAlpha(q)} On recommence l'expérience au début. Donner la probabilité d'obtenir une boule ${boules[choix2]}.` + ajouteChampTexteMathLive(exercice, NoQuestion + 1, "largeur15 inline") + "<br>"
  q++
  ...
  texteCorr = ""
  if (!exercice.interactif && !context.isAmc) {
    texteCorr += `${numAlpha(q)} L'issue «Pile-${boules[0]}» peut être obtenue de ${n1[0]} façon${n1[0] > 1 ? "s" : ""} et l'issue «Pile-${boules[1]}» peut être obtenue de ${n1[1]} façon${
      n1[1] > 1 ? "s" : ""
    }.<br>`
  ...
  texteCorr += `${numAlpha(q)} Comme on a fait ${pileOuFace}», on va tirer une boule dans la ${urne} urne où il y a ${nbBouleC} boule${nbBouleC > 1 ? "s" : ""} ${boules[choix1]}${
    nbBouleC > 1 ? "s" : ""
  } sur ${card} boules.<br>`
  ...
  texteCorr += `Il y a ${n1[choix2] + n2[choix2]} issues avec une boule ${boules[choix2]} sur ${card1 + card2} issues en tout. La probabilité de cet événement est donc de $${proba2.texFraction}${
    !proba2.estIrreductible ? "=" + proba2.texFractionSimplifiee : ""
  }$.<br>`
...
function urneDeuxTiragesAvecRemise(exercice, NoQuestion, sup, sup2, niveau) {
  ...
  let texte = `Dans une urne, il y a ${nbBoule1} boule${nbBoule1 > 1 ? "s" : ""} ${b1Color}${nbBoule1 > 1 && b1Char !== "O" ? "s" : ""} et ${nbBoule2} boule${nbBoule2 > 1 ? "s" : ""} ${b2Color}${
    nbBoule2 > 1 && b2Char !== "O" ? "s" : ""
  } indicernables au toucher.<br>`
  ...
  let texteCorr = `L'issue «${b1Color}-${b1Color}» peut être obtenue de ${contenu[0]} façon${contenu[0] > 1 ? "s" : ""} et l'issue «${b1Color}-${b2Color}» peut être obtenue de ${contenu[1]} façon${
    contenu[1] > 1 ? "s" : ""
  }.<br>`
  texteCorr += "Voici un tableau à double entrée qui représente toutes les issues de cette expérience.<br><br>"
  texteCorr += tableau + "<br><br>"
  ...
  texteCorr += `Une autre façon de faire est de considérer que c'est l'événement contraire de «obtenir deux boules de la même couleur» dont on a calculé la probabilité à la question ${numAlpha(
    1
  )}.<br>`
...
function urneDeuxTiragesSansRemise(exercice, NoQuestion, sup, sup2, niveau) {
  // tirage dans une urne avec remise
  const [b1Color, b2Color] = shuffle(["bleue", "rouge", "verte", "orange", "noire", "jaune"]).splice(0, 2)
