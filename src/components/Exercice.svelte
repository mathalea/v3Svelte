...
<div bind:this={divExercice}>
  ...
  >
    ...
    >
      {#if visible}
        <i class="bx bx-hide ml-6" />
      {:else}
        <i class="bx bx-show ml-6" />
      {/if}
    </button>
    ...
    >
      {#if contenuEtNonCorrection}
        <i class="bx bxs-toggle-left ml-6" />
      {:else}
        <i class="bx bxs-toggle-right ml-6" />
      {/if}
      {contenuEtNonCorrection ? "Correction" : "Consigne"}
    </button>
  </h1>
  {#if visible}
    {#if contenuEtNonCorrection}
...
<script lang="ts">
  ...
  $: {
    ...
    if (visible || contenuEtNonCorrection || !contenuEtNonCorrection)
      refreshDisplay();
  }
