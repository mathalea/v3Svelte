<script lang="ts">
  ...
  // petite demo de l'utilisation des dictionnaires
  import dico from "../modules/exercicesDisponiblesReferentiel2022.json";
  import refer from "../modules/referentiel2022.json";
  import uuids from "../modules/uuidsToUrl.json";
  const dictionnaire = toMap(dico);
  const referentiel2022 = toMap(refer);
  const uuidsToUrl = toMap(uuids);
  import {
    listeChapitresDuNiveau,
    listeExosDuChapitre,
    toMap,
    urlFromUuid,
    uuidFromRef,
  } from "../modules/fileTools";

  console.log(uuidFromRef("6C10", referentiel2022));
  console.log(listeExosDuChapitre("4C1", referentiel2022));
  console.log(listeChapitresDuNiveau("3e", referentiel2022));
  console.log(uuidsToUrl.get("b8acc")); // ou urlFromUuid("b8acc", uuidsToUrl)
// fin demo.
...
<div bind:this={divExercice}>
  ...
  >
    ...
    <div class="flex justify-end">
      <button type="button" on:click={switchInteractif}
        ><i class="bx ml-6 {interactif ? 'bxs-mouse' : 'bx-mouse'}" /></button
      >
      <button
        type="button"
        on:click={() => {
